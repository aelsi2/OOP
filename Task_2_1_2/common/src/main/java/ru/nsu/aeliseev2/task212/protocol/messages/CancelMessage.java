...
/**
 * Message sent by the client (master) to a server (slave) to cancel a work unit sent previously.
 *
 * @param id The id of the work unit.
 */
public record CancelMessage(long id) implements Message {
    private static final byte MESSAGE_TYPE = 2;
    private static final int MESSAGE_SIZE = 8;

    /**
     * Deserializer for {@code CancelMessage}.
     */
